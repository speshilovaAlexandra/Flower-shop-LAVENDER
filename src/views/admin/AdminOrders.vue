         ...
         <table class="admin-table">
           ...
           <tbody>
             <tr v-for="order in filteredOrders" :key="order.id">
               ...
               <td>
                 <div class="status-dropdown" @click.stop>
                   ...
                   >
                     ...
                     <svg class="dropdown-arrow" :class="{ 'rotated': activeDropdown === order.id }" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                       <polyline points="6 9 12 15 18 9"/>
                     </svg>
                   </button>
                   
                   <div v-if="activeDropdown === order.id" class="dropdown-menu" @click.stop>
                     <button 
                       v-for="status in statuses" 
                       :key="status.value"
                       @click.stop="updateStatus(order, status.value)"
                       class="dropdown-item"
               ...
               <td class="text-right">
                 <button @click="viewDetails(order)" class="btn-icon btn-view">Просмотр</button>
               </td>
             </tr>
           </tbody>
         </table>
           ...
           <div class="packages-section">
             ...
             <div v-for="(pkg, idx) in orderPackages" :key="idx" class="package-block">
               <div class="package-header">
                 ...
                 <span v-if="pkg.type === 'constructor'" class="badge-constructor">🧪 Конструктор</span>
                 <span v-else class="badge-catalog">🌺 Каталог</span>
               </div>
...
const activeDropdown = ref(null); // 🆕 Активный дропдаун
...
const orderPackages = computed(() => {
  ...
  return [{ 
    packaging: 'Не указана', 
    type: 'flower',
    items: selectedOrder.value.flowers?.map(f => ({ 
      id: f.id, 
...
onMounted(() => {
  // Добавляем обработчик клика вне дропдауна
...
const updateStatus = async (order, status) => {
  try { 
    ...
    activeDropdown.value = null; // Закрываем дропдаун
  } catch (e) { 
